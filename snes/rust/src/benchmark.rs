...
pub fn cpu_bench(c: &mut Criterion) {
    ...
    group.bench_function("nestest", |b| {
        b.iter_batched(
            || {
                let cpu: SnesCpu = SnesCpu::new();
                let ppu: SnesPpu = SnesPpu::new();
                let apu: SnesApu = SnesApu::new();
                let cpu_peripherals: SnesCpuPeripherals = SnesCpuPeripherals::new(ppu, apu);
                let mut mb: SnesMotherboard = SnesMotherboard::new();
                let nc = SnesCartridge::load_cartridge(
