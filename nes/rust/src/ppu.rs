...
impl<T> PixelImage<T> 
 where T: Default + Clone + Copy + std::cmp::PartialEq
{
    ...
    pub fn to_egui(&self, scale: Option<ScalingAlgorithm>) -> egui::ColorImage {
        ...
        let (pixels, width, height) = match scale {
            ...
            Some(alg) => match alg {
                ...
                ScalingAlgorithm::Scale3x => {
                    let mut newpixels = vec![egui::Color32::BLACK; pixels.len() * 9];
                    for y in 0..self.height {
                        for x in 0..self.width {
                            ...
                            let mut pg: [egui::Color32; 9] = [letters[4]; 9];
...
/// The access mode for the ppu
#[derive(serde::Serialize, serde::Deserialize, Copy, Clone, Debug, PartialEq)]
pub enum PpuMode {
    /// Fetch data related to the background
    Background,
    /// Fetch data related to sprites
    Sprite,
    /// Idle commands
    Idle,
}
...
impl NesPpu {
    ...
    /// Increment the vram address by a horizontal amount of 1
    ...
    fn increment_scanline_cycle(&mut self) {
        if self.should_render_background_cycle(8) || self.should_render_sprites(8) {
